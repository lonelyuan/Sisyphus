> [!warning] 归档说明 · 未来愿景，非当前契约
> **这是 KnownEngine 的原始设计基线（v0.1, 2026-07-16），归档于此作为知识工程"重装版 / CTF 拓展"的北极星，不是当前 skill 的操作契约。**
> - 当前实现与运行规则以 [SKILL.md](../SKILL.md) 及同目录其它 references 为准；本文件描述的 evidence-span / claim / 本体补丁队列 / 验证沙盒 / Writer Service / 多投影内核 **均尚未实现**，agent 日常沉淀**不要**照本文件行事。
> - 用途：当知识工程拓展到 **CTF 线下赛 / 科研文献调研**（需要证据链、可复现验证、离线混合检索、团队协作）时，回到这份文档做架构参考。
> - 当前实现有意做了瘦身（个人第二大脑场景），取舍理由见 [SKILL.md 未来方向](../SKILL.md) 与项目记忆。已吸收的轻量版：可靠性阶梯（[types.md](types.md)，本文件 §9.1 状态机的瘦身）、结晶化归并/拆分（[crystallization.md](crystallization.md)，本文件 §8.2 本体补丁的散文版）、分类均衡表述（[taxonomy.md](taxonomy.md)，本文件 §8.4）。
